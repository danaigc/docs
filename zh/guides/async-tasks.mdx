...
| Model | Type | Response |
|-------|------|----------|
| `nanobanana` | Synchronous | Results returned immediately |
| `nanobanana-pro` | Synchronous | Results returned immediately |
...
如果不想轮询，可以在创建任务时传入 `webhook_url`。当任务完成或失败时，Egg API 会向该地址发送 POST 通知。
