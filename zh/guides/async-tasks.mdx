...
如果不想轮询，可以在创建任务时传入 `webhook_url`。当任务完成或失败时，Egg API 会向该地址发送 POST 通知。可在 `https://webhook.site/` 获取测试回调地址。
...
- `X-EggAPI-Timestamp`: Unix 时间戳（秒）
- `X-EggAPI-Signature`: 可选。`sha256=` + HMAC-SHA256(hex)，签名内容为 `{timestamp}.{raw_body}`

<Note>
  只有在 Dashboard 里生成 Webhook Secret 后才会包含签名；否则不会发送 `X-EggAPI-Signature`。
</Note>
...
```json
{
  ...
  "model": "nanobanana-pro",
  "output": {
    "url": "https://cdn.eggapi.ai/outputs/abc123.png"
  },
  "error": null,
  "cost": "0.016000",
...
<Note>
  请使用原始请求体进行验签，避免对 JSON 进行重新序列化导致签名不一致。
</Note>

### 重试策略

- 2xx 视为成功响应
- 最多重试 5 次，指数退避（最大 4 小时）
- 单次请求超时 30 秒
...
console.log("Image URL:", task.output.url);
```
